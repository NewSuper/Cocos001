...
export class Ground extends Component {
    ...
    private m_MoveSwitch = false; //移动开关

    SetEnableMove(isMoveSwitch){
      this.m_MoveSwitch = isMoveSwitch
    }

    OnUpdate(deltaTime: number) {
        if(!this.m_MoveSwitch){ //没有开
           return
        }
    ...
    CheckBirdCollision(birdCtrl:BirdCtrl):boolean{
      // if(birdCtrl.node.position.y - birdCtrl.getHalfHeight()< this.node.position.y){
      //   return true   //this.node.position.y  锚点   birdCtrl.getHalfHeight()  图片的一半
      // }

      // if(birdCtrl.GetFootY() <　this.node.position.y){
      //   return  true
      // }    
      // return false    //鸟脚的位置小于我当前节点的位置，表示发生碰撞了
      // 可简写成下面  return birdCtrl.GetFootY() <　this.node.position.y
