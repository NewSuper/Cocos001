import { _decorator, assetManager, Component, Constructor, error, instantiate, Node, Prefab } from 'cc';
...
export class UIMgr extends Component {
    ...
    open<UIDATA = any,T extends (Component & IUI<UIDATA>) = any>(uiClass:UIClass<T,UIDATA>,uiData?: UIDATA){
      assetManager.loadBundle(uiClass.bundleName,(_,bundle) =>{
         //assetManager.loadBundle("PrefabBundle",(_,bundle) =>{
         //bundle.load("LoseUI",Prefab,(err,prefab) =>{
         bundle.load(uiClass.prefabUrl,Prefab,(err,prefab) =>{
           ...
           // const uiComponent = uiNode.getComponent("LoseUI") as any as IUI<UIDATA>
